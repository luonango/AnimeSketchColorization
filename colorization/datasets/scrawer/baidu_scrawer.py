...
class BaiduImgDownloader(object):
    ...
    def __init__(self, word, dirpath=None, processNum=30, max_num_pic=10000):
        ...
        pre_root_path_raw = os.path.join(os.environ['HOME'],
    ...
    def __buildUrls(self):
        ...
        maxNum = maxNum if maxNum < int(self.MAX_NUM_PIC * 12) else int(self.MAX_NUM_PIC * 12)
    ...
    def __resolveImgUrl(self, url):
        ...
        try:
            ...
            if self.NUM_PIC > self.MAX_NUM_PIC:
                ...
                self.messageQueue.put(self.printPrefix + u"已解析出 %s 个图片网址" % len(imgs))
                self.queue.put(imgs)
...
if __name__ == '__main__':
    print(u"欢迎使用百度图片下载脚本！\n目前仅支持单个关键词。")
    print(u"下载结果保存在脚本目录下的anime/raw文件夹中。")
    print("=" * 50)
